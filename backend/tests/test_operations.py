...
def test_daily_runner_persists_counts_and_safe_failure(
    ...
    repeated = asyncio.run(runner.run(PipelineTriggerType.MANUAL))
    assert repeated.status is PipelineStatus.SUCCEEDED
    ...
    try:
        ...
        assert connection.execute("SELECT COUNT(*) FROM daily_reports").fetchone()[0] == 1
    finally:
        connection.close()
