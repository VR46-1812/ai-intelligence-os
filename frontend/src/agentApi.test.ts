...
describe("agent API", () => {
  ...
  it("accepts degraded source state", async () => {
    const fetcher = vi.fn(() => Promise.resolve(new Response(JSON.stringify({ pipeline_run_id: "run-1", current_agent: null, latest_success_at: "2026-07-22T00:00:00Z", executions: [{ id: "execution-1", agent_id: "orchestrator", stage_order: 1, status: "succeeded", attempt: 1, input: { report_date: "2026-07-22" }, output: { summary: "Completed" }, evidence_refs: [], provenance_refs: [], metrics: { duration_ms: 4 }, safe_failure_reason: null, started_at: "2026-07-22T00:00:00Z", completed_at: "2026-07-22T00:00:00Z" }], degraded_sources: ["openreview"] }), { status: 200 }))) as unknown as typeof fetch;
